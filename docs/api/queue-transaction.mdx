...
Receives a **signed SafeTx proposal** from the client. The server hard-validates it (signature recovery against the recorded owner set, threshold arithmetic, nonce/hash consistency), mirrors it to the **Safe Transaction Service** — so it appears in app.safe.global at 1 of 2 — then enqueues a screening job for the agent runtime.
